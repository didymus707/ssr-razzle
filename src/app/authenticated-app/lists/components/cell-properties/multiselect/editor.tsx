...
export const MultiSelectCellEditor = (props: Props) => {
  const { updateValue, updateSelectOption, value, allowCreate, optionItemStyles = {} } = props;
  const options = props.options.filter((option: any) => !option.isDeleted);

  const selectOption = (option_id: string | number) => {
    if (!Array.isArray(value) && !value) updateValue([option_id]);
    if (!Array.isArray(value) && value) updateValue([value, option_id]);
    if (value.includes(option_id)) return;
    updateValue([...value, option_id]);
  };
